...
public class ColorUtils {
    ...
    public static int randomColor(int minR, int maxR, int minG, int maxG, int minB, int maxB) {
        return 0xff000000 |
                ((minR + TorchesBecomeSunlight.random.nextInt(maxR + 1 - minR)) << 16) |
                ((minG + TorchesBecomeSunlight.random.nextInt(maxG + 1 - minG)) << 8) |
                ((minB + TorchesBecomeSunlight.random.nextInt(maxB + 1 - minB))) ;
    }

    public static int randomColor(int minA, int maxA, int minR, int maxR, int minG, int maxG, int minB, int maxB) {
        return  ((minR + TorchesBecomeSunlight.random.nextInt(maxA + 1 - minA)) << 24) |
                ((minR + TorchesBecomeSunlight.random.nextInt(maxR + 1 - minR)) << 16) |
                ((minG + TorchesBecomeSunlight.random.nextInt(maxG + 1 - minG)) << 8) |
                ((minB + TorchesBecomeSunlight.random.nextInt(maxB + 1 - minB))) ;
    }
