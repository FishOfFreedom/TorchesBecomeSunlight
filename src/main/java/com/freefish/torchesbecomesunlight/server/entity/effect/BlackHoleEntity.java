...
public class BlackHoleEntity extends Entity implements GeoEntity {
    ...
    private void linkEntity(){
        List<LivingEntity> list = level().getEntitiesOfClass(LivingEntity.class,getBoundingBox().inflate(8),livingEntity ->
                livingEntity != owner&&livingEntity.distanceTo(this)<6+livingEntity.getBbWidth()/2);
        for(LivingEntity livingEntity:list){
            Vec3 move = position().subtract(livingEntity.position()).scale(0.5);
