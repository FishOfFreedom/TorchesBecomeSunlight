...
public class BigIceCrystal extends AbstractArrow implements GeoEntity {
    ...
    public BigIceCrystal(Level pLevel, LivingEntity pShooter) {
        super(EntityRegistry.BIG_ICE_CRYSTAL.get(), pShooter, pLevel);;
    }
        ...
        for(int i = 0;i<5;i++){
            for(int j = 0;j<5;j++){
                ...
                for(int i1 = 0;i1<6;i1++){
                    ...
                    if(blockState.is(BlockRegistry.ORE_AND_ICE.get())) flag = true;
                    if(!blockState.isAir()&& !blockState.is(Blocks.SNOW))
                        count = i1;
                }
                if(count==-1||count==5||flag) continue;
                BlockPos blockpos = blockPos1.above(count+1);
                BlockState blockstate2 = BlockRegistry.ORE_AND_ICE.get().defaultBlockState();
        ...
        for(int i = 0;i<5;i++){
            for(int j = 0;j<5;j++){
                ...
                for(int i1 = 0;i1<6;i1++){
                    ...
                    if(blockState.is(BlockRegistry.ORE_AND_ICE.get())){
        ...
        for(LivingEntity livingEntity1:livingEntity){
            ...
            if(tickCount%5==0) {
                ...
                if(caster instanceof SnowNova snowNova){
                    ...
                    livingEntity1.hurt(DamageSourceRegistry.SnowMonsterFrozen(snowNova),damage/2);
