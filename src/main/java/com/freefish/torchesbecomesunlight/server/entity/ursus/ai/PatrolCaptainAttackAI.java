package com.freefish.torchesbecomesunlight.server.entity.ursus.ai;
...
public class PatrolCaptainAttackAI extends Goal {
    private PatrolCaptain mangler;
    ...
    public PatrolCaptainAttackAI(PatrolCaptain mangler){
    ...
    public void tick() {
        ...
        if (target.getY() - this.mangler.getY() >= -1 && target.getY() - this.mangler.getY() <= 3) {
            if (dist < 2&& Math.abs(MathUtils.wrapDegrees(this.mangler.getAngleBetweenEntities(target, this.mangler) - this.mangler.yBodyRot)) < 35.0D) {
                ...
                if(v>0.5f)
                    AnimationActHandler.INSTANCE.sendAnimationMessage(this.mangler, PatrolCaptain.DASHATTACK);
                else
                    AnimationActHandler.INSTANCE.sendAnimationMessage(this.mangler, PatrolCaptain.ATTACK);
            }
        }
    }
