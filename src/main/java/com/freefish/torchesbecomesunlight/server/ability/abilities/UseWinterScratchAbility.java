...
public class UseWinterScratchAbility extends PlayerAbility {

    public UseWinterScratchAbility(AbilityType<Player, UseWinterScratchAbility> abilityType, Player user) {
        ...
        if(!level.isClientSide){
            if (getTicksInUse() == 5) {
                for(int i = 0;i<5;i++){
                    ...
                    IceCrystal.spawnWaitCrystal(player.level(), new Vec3(4 - random.nextInt(9), 2 + random.nextInt(5), 4 - random.nextInt(9)).add(player.position()), player, FFEntityUtils.getHeadRotVec(player, new Vec3(0, 0, 10)).add(0,player.getEyeHeight(),0));
                }
            }
