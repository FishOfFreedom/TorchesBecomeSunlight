...
public class TBSJigsawPlacement {
      ...
      if (structurepoolelement == EmptyPoolElement.INSTANCE) {
         ...
         return Optional.of(new Structure.GenerationStub(new BlockPos(i, i1, j), (structurePiecesBuilder) -> {
            ...
            if (level > 0) {
               ...
               if(key!=null){
                  RosmontisLib.LOGGER.info(key + " at " + blockpos);
   ...
   private static void addPieces(RandomState p_227211_, int pMaxDepth, boolean p_227213_, ChunkGenerator pChunkGenerator, StructureTemplateManager pStructureTemplateManager, LevelHeightAccessor p_227216_, RandomSource pRandom, Registry<StructureTemplatePool> pPools, PoolElementStructurePiece p_227219_, List<PoolElementStructurePiece> pPieces, VoxelShape p_227221_) {
      TBSJigsawPlacement.Placer jigsawplacement$placer = new TBSJigsawPlacement.Placer(pPools, pMaxDepth, pChunkGenerator, pStructureTemplateManager, pPieces, pRandom);
      jigsawplacement$placer.placing.addLast(new TBSJigsawPlacement.PieceState(p_227219_, new MutableObject<>(p_227221_), 0));

      while(!jigsawplacement$placer.placing.isEmpty()) {
         TBSJigsawPlacement.PieceState jigsawplacement$piecestate = jigsawplacement$placer.placing.removeFirst();
                              ...
                              if (JigsawBlock.canAttach(structuretemplate$structureblockinfo, structuretemplate$structureblockinfo1)) {
                                 ...
                                 if (!Shapes.joinIsNotEmpty(mutableobject1.getValue(), Shapes.create(AABB.of(boundingbox3).deflate(0.25D)), BooleanOp.ONLY_SECOND)) {
                                    ...
                                    if (pDepth + 1 <= this.maxDepth) {
                                       this.placing.addLast(new TBSJigsawPlacement.PieceState(poolelementstructurepiece, mutableobject1, pDepth + 1));
                                    }
                                    continue label129;
                                 }
                              }
