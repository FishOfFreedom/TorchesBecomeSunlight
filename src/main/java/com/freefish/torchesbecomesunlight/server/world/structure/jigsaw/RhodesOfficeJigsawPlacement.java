package com.freefish.torchesbecomesunlight.server.world.structure.jigsaw;

import com.freefish.rosmontislib.RosmontisLib;
import com.freefish.rosmontislib.levelentity.LevelEntityManager;
import com.freefish.torchesbecomesunlight.TorchesBecomeSunlight;
import com.freefish.torchesbecomesunlight.mixin.accessor.ChunkAccessAccessor;
import com.freefish.torchesbecomesunlight.mixin.accessor.PoolElementStructurePieceAccessor;
...
import com.freefish.torchesbecomesunlight.server.world.structure.FFStructures;
import com.freefish.torchesbecomesunlight.server.world.structure.MyStructureGenerator;
...
public class RhodesOfficeJigsawPlacement {

   public static Optional<Structure.GenerationStub> addPieces(Structure.GenerationContext pContext, Holder<StructureTemplatePool> poolHolder, Optional<ResourceLocation> startJigsawName, int level, BlockPos blockPos, boolean p_227244_, Optional<Heightmap.Types> heightMap, int maxDistanceFromCenter, FFStructures ffStructures) {
      ...
      if (structurepoolelement == EmptyPoolElement.INSTANCE) {
         ...
         if (startJigsawName.isPresent()) {
            ...
            if (optional.isEmpty()) {
               TorchesBecomeSunlight.LOGGER.error("No starting jigsaw {} found in start pool {}", resourcelocation, poolHolder.unwrapKey().map((p_248484_) -> {
         ...
         if (heightMap.isPresent()) {
            k = blockPos.getY() + chunkgenerator.getFirstFreeHeight(i, j, heightMap.get(), levelheightaccessor, pContext.randomState());
         } else {
            k = blockpos1.getY();
         }
         ...
         int i1 = k + vec3i.getY();
         return Optional.of(new Structure.GenerationStub(new BlockPos(i, i1, j), (structurePiecesBuilder) -> {
            ...
            if (level > 0) {
               ...
               ResourceLocation key = registryaccess.registryOrThrow(Registries.STRUCTURE_TYPE).getKey(ffStructures.type());

               if(key!=null){
                  BlockPos blockPos1 = new BlockPos(blockPos.getX(), k, blockPos.getZ());
                  RosmontisLib.LOGGER.info(key + " at " + blockPos1);
                  if(levelheightaccessor instanceof ChunkAccess level1){
                     LevelHeightAccessor levelHeightAccessor = ((ChunkAccessAccessor) level1).getLevelHeightAccessor();
                     if(levelHeightAccessor instanceof ServerLevel level2){
                        LevelEntityManager instance = LevelEntityManager.getInstance(level2);
                        RhodesIslandLevelEntity instanceLevelEntity = instance.getInstanceLevelEntity(TBSLevelEntityHandle.RHODES_ISLAND_DATA);
                        instanceLevelEntity.add(blockPos1.offset(17, 1, 11), 64, 0.5f);
                     }
                  }

                  MyStructureGenerator.markStructureGenerated(key, blockPos);
               }

               VoxelShape voxelshape = Shapes.join(Shapes.create(aabb), Shapes.create(AABB.of(boundingbox)), BooleanOp.ONLY_FIRST);
               addPieces(pContext.randomState(), level, p_227244_, chunkgenerator, structuretemplatemanager, levelheightaccessor, worldgenrandom, registry, poolelementstructurepiece, list, voxelshape);
   ...
   private static void addPieces(RandomState p_227211_, int pMaxDepth, boolean p_227213_, ChunkGenerator pChunkGenerator, StructureTemplateManager pStructureTemplateManager, LevelHeightAccessor p_227216_, RandomSource pRandom, Registry<StructureTemplatePool> pPools, PoolElementStructurePiece p_227219_, List<PoolElementStructurePiece> pPieces, VoxelShape p_227221_) {
      RhodesOfficeJigsawPlacement.Placer jigsawplacement$placer = new RhodesOfficeJigsawPlacement.Placer(pPools, pMaxDepth, pChunkGenerator, pStructureTemplateManager, pPieces, pRandom);
      jigsawplacement$placer.placing.addLast(new RhodesOfficeJigsawPlacement.PieceState(p_227219_, new MutableObject<>(p_227221_), 0));

      while(!jigsawplacement$placer.placing.isEmpty()) {
         RhodesOfficeJigsawPlacement.PieceState jigsawplacement$piecestate = jigsawplacement$placer.placing.removeFirst();
   ...
   static final class Placer {
      ...
      void tryPlacingChildren(PoolElementStructurePiece pPiece, MutableObject<VoxelShape> p_227266_, int pDepth, boolean p_227268_, LevelHeightAccessor p_227269_, RandomState p_227270_) {
         ...
         for(StructureTemplate.StructureBlockInfo structuretemplate$structureblockinfo : structurepoolelement.getShuffledJigsawBlocks(this.structureTemplateManager, blockpos, rotation, this.random)) {
            ...
            if (optional.isEmpty()) {
               TorchesBecomeSunlight.LOGGER.warn("Empty or non-existent pool: {}", (Object)resourcekey.location());
            } else {
               ...
               if (holder.value().size() == 0 && !holder.is(Pools.EMPTY)) {
                  ...
                  if (holder1.value().size() == 0 && !holder1.is(Pools.EMPTY)) {
                     TorchesBecomeSunlight.LOGGER.warn("Empty or non-existent fallback pool: {}", holder1.unwrapKey().map((p_255599_) -> {
                           ...
                           for(StructureTemplate.StructureBlockInfo structuretemplate$structureblockinfo1 : list1) {
                              if (JigsawBlock.canAttach(structuretemplate$structureblockinfo, structuretemplate$structureblockinfo1)) {
                                 ...
                                 if (!Shapes.joinIsNotEmpty(mutableobject1.getValue(), Shapes.create(AABB.of(boundingbox3).deflate(0.25D)), BooleanOp.ONLY_SECOND)) {
                                    ...
                                    StructurePoolElement element = ((PoolElementStructurePieceAccessor) poolelementstructurepiece).getElement();
                                    if(element instanceof SinglePoolElement singlePoolElement){
                                       //System.out.println(singlePoolElement.toString());
                                    }

                                    this.pieces.add(poolelementstructurepiece);
                                    if (pDepth + 1 <= this.maxDepth) {
                                       this.placing.addLast(new RhodesOfficeJigsawPlacement.PieceState(poolelementstructurepiece, mutableobject1, pDepth + 1));
                                    }
                                    continue label129;
                                 }
                              }
